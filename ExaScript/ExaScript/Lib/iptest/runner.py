...
def run_test_pkg(pkg_name, do_not_run=[]):
    ...
    subpkg_list = [x for x in nt.listdir(pkg_dir_name) if not x.endswith(".esc") and file_exists(pkg_dir_name + "\\" + x + "\\__init__.py")]
    log_debug("Subpackages found: %s" % (str(subpkg_list)))
    module_list = [x for x in nt.listdir(pkg_dir_name) if x.endswith(".esc") and x!="__init__.esc"]
    ...
    for test_module in module_list:
        test_module = pkg_name + "." + test_module.split(".esc", 1)[0]
